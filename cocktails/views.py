...
def user_cocktails(request):
    ...
    return render(
        request,
        "cocktails/user_cocktails.html",
        {"user": request.user, "cocktails": cocktails},
    )
...
def create_cocktail(request):
    ...
    return render(
        request,
        "cocktails/create_cocktail.html",
        {
            "form": form,
            "formset": formset,
        },
...
def starred_cocktails(request):
    favourites = UsersFavouriteCocktails.objects.filter(user=request.user).values_list(
        "cocktail", flat=True
    )
    cocktails = Cocktail.objects.filter(id__in=favourites)
    return render(
        request, "cocktails/user_favourite_cocktails.html", {"cocktails": cocktails}
    )
...
def ingredients_list(request):
    ...
    return render(
        request, "ingredients/ingredients_list.html", {"ingredients": ingredients}
    )
...
def cocktail_details(request, pk):
    cocktail = get_object_or_404(Cocktail, pk=pk)
    ingredients = CocktailIngrediets.objects.filter(cocktail=cocktail).select_related(
        "ingredient"
    )
    return render(
        request,
        "cocktails/cocktail_details.html",
        {
            "cocktail": cocktail,
...
def delete_ingredient(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            return render(
                request,
                "ingredients/delete_ingredient.html",
                {
        ...
        for cocktail in cocktails:
            ...
            for ci in ingredients_qs:
                ingredients_list.append(
                    f"{ci.ingredient.name} ({ci.quantity} {ci.unit})"
                )
